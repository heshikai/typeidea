...
class PostInline(admin.StackedInline):#用一下StatckedInline这种样式，看下不同的样式
    fields = ('title','desc')
    extra = 0 #控制额外多几个
    model = Post
...
class CategoryAdmin(BaseOwnerAdmin):
    list_display = (
        'name','status','is_nav','created_time','owner','post_count'
    )
...
class LogEntryAdmin(admin.ModelAdmin):
    ...
    list_display = ['object_repr','content_type_id','object_id','action_flag',
                    'user','change_message',]
    list_filter = ['content_type_id',]

    '''def get_queryset(self,request):
        qs=super(LogEntryAdmin,self).get_queryset(request)
        return qs.filter(content_type_id=1)'''